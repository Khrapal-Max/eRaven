...
public sealed class TimesheetPolicyRepositoryTests
{
    ...
    [Fact]
    public async Task Throws_when_code_is_null_or_whitespace()
    {
        await using var tdb = new SqliteTestDb();
        var repo = new TimesheetPolicyRepository(tdb.Factory);

        var ex = await Assert.ThrowsAsync<ArgumentException>(() =>
                repo.GetAllowedTransitionOptionsAsync("   "));

        Assert.Equal("code", ex.ParamName);
    }

    [Fact]
    public async Task Throws_when_code_not_found_or_inactive()
    {
        await using var tdb = new SqliteTestDb();
        var repo = new TimesheetPolicyRepository(tdb.Factory);
        ...
        using (var db = tdb.Factory.CreateDbContext())
        {
            db.Add(new TimesheetCodeDefinition
            {
                Id = Guid.NewGuid(),
                Code = "X",
                Title = "Some",
        ...
        var ex = await Assert.ThrowsAsync<ArgumentException>(() =>
                 repo.GetAllowedTransitionOptionsAsync("MISSING"));
    ...
    [Fact]
    public async Task Returns_only_active_non_NB_transitions_ordered_and_projected()
    {
        await using var tdb = new SqliteTestDb();
        var repo = new TimesheetPolicyRepository(tdb.Factory);
        ...
        // codes
        var from = new TimesheetCodeDefinition
        {
            Id = fromId,
            Code = "FROM",
            Title = "From",
        ...
        // очікувані (активні, не НБ)
        var to30 = new TimesheetCodeDefinition { Id = Guid.NewGuid(), Code = "30", Title = "T30", SortOrder = 0, Priority = 9, IsActive = true, CreatedAtUtc = DateTime.UtcNow };
        var to15 = new TimesheetCodeDefinition { Id = Guid.NewGuid(), Code = "15", Title = "T15", SortOrder = 1, Priority = 1, IsActive = true, CreatedAtUtc = DateTime.UtcNow };
        var to20 = new TimesheetCodeDefinition { Id = Guid.NewGuid(), Code = "20", Title = "T20", SortOrder = 1, Priority = 1, IsActive = true, CreatedAtUtc = DateTime.UtcNow };
        var to10 = new TimesheetCodeDefinition { Id = Guid.NewGuid(), Code = "10", Title = "T10", SortOrder = 1, Priority = 2, IsActive = true, CreatedAtUtc = DateTime.UtcNow };

        // мають бути відфільтровані
        var toInactive = new TimesheetCodeDefinition { Id = Guid.NewGuid(), Code = "INACT", Title = "Inactive", SortOrder = 0, Priority = 0, IsActive = false, CreatedAtUtc = DateTime.UtcNow };
        var toNb = new TimesheetCodeDefinition { Id = Guid.NewGuid(), Code = TimesheetSystemCodes.NotInTimesheet, Title = "NotInTimesheet", SortOrder = 0, Priority = 0, IsActive = true, CreatedAtUtc = DateTime.UtcNow };

        using (var db = tdb.Factory.CreateDbContext())
        {
            db.TimesheetCodes.AddRange(from, to30, to15, to20, to10, toInactive, toNb);

            db.TimesheetCodeTransitions.AddRange(
                new TimesheetCodeTransition { Id = Guid.NewGuid(), FromCodeId = fromId, ToCodeId = to30.Id, StartShiftDays = 1, CreatedAtUtc = DateTime.UtcNow },
                new TimesheetCodeTransition { Id = Guid.NewGuid(), FromCodeId = fromId, ToCodeId = to15.Id, StartShiftDays = 0, CreatedAtUtc = DateTime.UtcNow },
                new TimesheetCodeTransition { Id = Guid.NewGuid(), FromCodeId = fromId, ToCodeId = to20.Id, StartShiftDays = 1, CreatedAtUtc = DateTime.UtcNow },
                new TimesheetCodeTransition { Id = Guid.NewGuid(), FromCodeId = fromId, ToCodeId = to10.Id, StartShiftDays = 0, CreatedAtUtc = DateTime.UtcNow },

                // filtered
                new TimesheetCodeTransition { Id = Guid.NewGuid(), FromCodeId = fromId, ToCodeId = toInactive.Id, StartShiftDays = 0, CreatedAtUtc = DateTime.UtcNow },
                new TimesheetCodeTransition { Id = Guid.NewGuid(), FromCodeId = fromId, ToCodeId = toNb.Id, StartShiftDays = 0, CreatedAtUtc = DateTime.UtcNow }
            );
        ...
        // act
        var result = await repo.GetAllowedTransitionOptionsAsync("FROM");

        // assert: тільки 4, відсортовано: SortOrder -> Priority -> Code
        Assert.Equal(4, result.Count);

        Assert.Collection(result,
            r =>
            {
                Assert.Equal("30", r.Code);
                Assert.Equal("T30", r.Title);
                Assert.Equal(1, r.StartShiftDays);
            },
            r =>
            {
                Assert.Equal("15", r.Code);
                Assert.Equal("T15", r.Title);
                Assert.Equal(0, r.StartShiftDays);
            },
            r =>
            {
                Assert.Equal("20", r.Code);
                Assert.Equal("T20", r.Title);
                Assert.Equal(1, r.StartShiftDays);
            },
            r =>
            {
                Assert.Equal("10", r.Code);
                Assert.Equal("T10", r.Title);
                Assert.Equal(0, r.StartShiftDays);
            });
    }
}
