...
{
    // Отримати місячний табель по всім особам
    ...
    // Отримати денний табель по всім особам
    Task<IReadOnlyList<TimesheetDayPerPersonCurrentStateDto>> GetDailyTimesheetAsync(DateOnly date, string? search, EnrollmentKind? enrollmentKind = null,
       bool activeOnly = true, CancellationToken ct = default);

    // Отримати місячний табель певної особи
    Task<TimesheetPersonMonthDto?> GetPersonTimesheetMonthAsync(Guid personId, int year, int month, CancellationToken ct = default);
