...
public sealed class MissionAssignmentConfiguration : IEntityTypeConfiguration<MissionAssignment>
{
    ...
    {
        e.ToTable("mission_assignments", tb =>
        {
            // half-open interval invariant: To is exclusive and must be > From when present.
            tb.HasCheckConstraint(
                "ck_mission_assignments_to_gt_from",
                "to_date IS NULL OR to_date > from_date");

            // closed interval must have End source ids; open interval must not have them.
            tb.HasCheckConstraint(
                "ck_mission_assignments_end_source_consistency",
                "(to_date IS NULL AND source_end_document_id IS NULL AND source_end_details_id IS NULL) OR " +
                "(to_date IS NOT NULL AND source_end_document_id IS NOT NULL AND source_end_details_id IS NOT NULL)");
        });
        ...
        // Idempotency: each DetailsId can be applied at most once.
        e.HasIndex(x => x.SourceStartDetailsId)
            .HasDatabaseName("ux_mission_assignments_start_details")
            .IsUnique();

        e.HasIndex(x => x.SourceEndDetailsId)
            .HasDatabaseName("ux_mission_assignments_end_details")
            .IsUnique()
            .HasFilter("source_end_details_id IS NOT NULL");
