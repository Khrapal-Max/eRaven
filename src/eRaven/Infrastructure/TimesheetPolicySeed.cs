    ...
    public static async Task EnsureSeededAsync(AppDbContext db, CancellationToken ct)
    {
        ...
        if (!await db.TimesheetCodes.AnyAsync(ct))
        {
            ...
            var codes = new List<TimesheetCodeDefinition>
            {
                ...
                // базовий факт-стан
                Code("0",  "Рекрут",   10, TimesheetEndDateMeaning.LastDayOfThisCode),
        ...
        if (!await db.TimesheetCodeTransitions.AnyAsync(ct))
        {
            ...
            // Рекрут дозволені: ВДР, 30, ЛХ, СЗЧ, 200, А, БВ (СЗЧ)
            AddFrom("0", "ВДР", "30", "ЛХ", "СЗЧ", "200", "А", "БВ (СЗЧ)");

            // ВДР дозволені: 30, ЛХ, СЗЧ, 200, А, БВ (СЗЧ)
            AddFrom("ВДР", "30", "ЛХ", "СЗЧ", "200", "А", "БВ (СЗЧ)");

            // ВП: 30, ЛХ, СЗЧ, 200, А, БВ (СЗЧ)
            AddFrom("ВП", "30", "ЛХ", "СЗЧ", "200", "А", "БВ (СЗЧ)");

            // ВПХ: 30, ЛХ, СЗЧ, 200, А, БВ (СЗЧ), ВЛК
            AddFrom("ВПХ", "30", "ЛХ", "СЗЧ", "200", "А", "БВ (СЗЧ)", "ВЛК");

            // ВПП: 30, ЛП, СЗЧ, 200, А, БВ (СЗЧ), ВЛК
            AddFrom("ВПП", "30", "ЛП", "СЗЧ", "200", "А", "БВ (СЗЧ)", "ВЛК");

            // ВЛК: 30, ЛХ, СЗЧ, 200, А, БВ (СЗЧ), ЛП, ВПХ, ВПП
            AddFrom("ВЛК", "30", "ЛХ", "СЗЧ", "200", "А", "БВ (СЗЧ)", "ЛП", "ВПХ", "ВПП");

            // МСЕК: 30
            AddFrom("МСЕК", "30");

            // ЛХ: 30, ВПХ, СЗЧ, 200, А, БВ (СЗЧ), ВЛК, РОЗПОР
            AddFrom("ЛХ", "30", "ВПХ", "СЗЧ", "200", "А", "БВ (СЗЧ)", "ВЛК", "РОЗПОР");

            // ЛП: 30, ВПП, СЗЧ, 200, А, БВ (СЗЧ), ВЛК, РОЗПОР
            AddFrom("ЛП", "30", "ВПП", "СЗЧ", "200", "А", "БВ (СЗЧ)", "ВЛК", "РОЗПОР");

            // БВ: 30, П, СЗЧ, 200, БВ (СЗЧ), РОЗПОР
            AddFrom("БВ", "30", "П", "СЗЧ", "200", "БВ (СЗЧ)", "РОЗПОР");
            ...
            AddFrom("БВ (СЗЧ)", "30", "РОЗПОР", "СЗЧ", "200");

            // СЗЧ: 30, РОЗПОР
            AddFrom("СЗЧ", "30", "РОЗПОР");
            ...
            AddFrom("100", "30", "БВ", "ПБД", "200", "БВ (СЗЧ)", "П", "Ф100");

            // Ф100: 30, ЛП, ЛХ
            AddFrom("Ф100", "30", "ЛП", "ЛХ");
