...
public sealed class TimeSheetAggregate
{
        ...
        DateTime nowUtc)
    {
        ...
        if (nextCodeId == Guid.Empty)
            throw new InvalidOperationException("CodeId is required.");
    ...
    public void SoftDeleteTimesheetEntry(Guid entryId, string reason, string author, DateTime nowUtc)
    {
        ...
        // NOTE: після CloseEpisode в епізоді можуть лишитися soft-deleted майбутні події (audit).
        // Для видалення/корекцій не блокуємося їх датою, але не дозволяємо вихід нижче OpenedAt.
        if (entry.From < OpenedAt)
            throw new InvalidOperationException($"Entry date {entry.From} is before OpenedAt {OpenedAt}.");
    ...
    public void RemoveTimesheetEntry(Guid entryId)
    {
        ...
        // Дозволяємо прибирати soft-deleted майбутні записи навіть у закритому епізоді.
        // Але не дозволяємо виходити нижче OpenedAt.
        if (entry.From < OpenedAt)
            throw new InvalidOperationException($"Entry date {entry.From} is before OpenedAt {OpenedAt}.");
